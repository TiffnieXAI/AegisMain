...
def build_llm_context(
    ...
    for vuln in vulnerabilities:
        ...
        lines.append(
            f"[HISTORICAL_EXPLOIT_MATCH]: {vuln.text} | SEVERITY: {severity}")
        lines.append(f"[SEARCH_RELEVANCE]: {vuln.match_confidence}")

    if not vulnerabilities:
        lines.append(
            "[HISTORICAL_EXPLOIT_MATCH]: No historical exploit match found.")

    for scam in scam_matches:
        scam_type = scam.severity.upper() if scam.severity else "UNKNOWN"
        lines.append(
            f"[SCAM_PATTERN_MATCH]: {scam.text} | SEVERITY: {scam_type}")
...
async def analyze_intent(report: SimulationReport) -> AnalysisResponse:
    ...
    if _embedder is None or _collection is None:
        raise HTTPException(
            status_code=503, detail="A.E.G.I.S. backend not initialised.")
    ...
    has_any_match = bool(
        all_vuln_hits or all_scam_hits) and top_score >= MIN_CONFIDENCE
