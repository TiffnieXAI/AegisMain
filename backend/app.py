    ...
    try:
        ...
        if result and result.value:
            ...
            if 'Raw' in display_data:
                ...
                name = bytes.fromhex(raw_name[2:]).decode('utf-8') if raw_name.startswith('0x') else raw_name
...
async def analyze_intent(tx_payload: TransactionRequest):
    ...
    try:
        status_code = registry_contract.functions.checkAddress(target_addr).call()
    except Exception:
        status_code = 0
